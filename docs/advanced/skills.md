...
```bash
php artisan oi:skills oilab-laravel-notes --project
# or, for the global scope (~/.claude)
php artisan oi:skills oilab-laravel-notes --global
```
...
> A package-local command `php artisan oi-notes:install-ai-skill` is still available for projects that don't use
